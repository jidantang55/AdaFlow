...
def register_pivotal(diffusion_model, is_pivotal, pivotal_idx=None):
    for _, module in diffusion_model.named_modules():
        # If for some reason this has a different name, create an issue and I'll fix it
        if isinstance_str(module, "BasicTransformerBlock"):
            setattr(module, "pivotal_pass", is_pivotal)
            if is_pivotal:
                setattr(module, "pivotal_idx", pivotal_idx)


def register_batch_idx(diffusion_model, batch_idx, batch_start_idx=0):
    for _, module in diffusion_model.named_modules():
        # If for some reason this has a different name, create an issue and I'll fix it
        if isinstance_str(module, "BasicTransformerBlock"):
...
def register_matching(diffusion_model, matching, dift_size, key_matching):
    for _, module in diffusion_model.named_modules():
        # If for some reason this has a different name, create an issue and I'll fix it
        if isinstance_str(module, "BasicTransformerBlock"):
            setattr(module, "matching", matching)
            setattr(module, "dift_size", dift_size)
            setattr(module.attn1, "key_matching", key_matching)
            setattr(module.attn1, "dift_size", dift_size)
    ...
    def sa_forward(self):
        ...
        def forward(x, encoder_hidden_states=None, attention_mask=None):
            ...
            if n_frames > max_batch_size:
                ...
                assert selected_indices.shape[1] == num_selected_tokens

                k_source = k[:n_frames]
                # k_uncond = k[n_frames:2 * n_frames].view(-1, k_source.shape[-1])[selected_indices].reshape(1, num_selected_tokens, -1).repeat(n_frames, 1, 1)
                # k_uncond = torch.cat([k[n_frames:2 * n_frames], k_uncond], dim=1)
                # k_cond = k[2 * n_frames:].view(-1, k_source.shape[-1])[selected_indices].reshape(1, num_selected_tokens, -1).repeat(n_frames, 1, 1)
                # k_cond = torch.cat([k[2 * n_frames:], k_cond], dim=1)
                k_uncond = k[n_frames:2 * n_frames].reshape(1, n_frames * sequence_length, -1)
                ...
                # v_uncond = torch.cat([v[n_frames:2 * n_frames], v_uncond], dim=1)
                # v_cond = v[2 * n_frames:].view(-1, v_source.shape[-1])[selected_indices].reshape(1, num_selected_tokens, -1).repeat(n_frames, 1, 1)
                # v_cond = torch.cat([v[2 * n_frames:], v_cond], dim=1)
                v_uncond = v[n_frames:2 * n_frames].reshape(1, n_frames * sequence_length, -1)
                v_uncond = torch.stack([v_uncond[0][selected_indices[i]] for i in range(n_frames)], dim=0)
                v_cond = v[2 * n_frames:].reshape(1, n_frames * sequence_length, -1)
                v_cond = torch.stack([v_cond[0][selected_indices[i]] for i in range(n_frames)], dim=0)
                # num_selected_tokens = num_selected_tokens + sequence_length
                ...
                num_selected_tokens = n_frames * sequence_length
            ...
            k_uncond = k_uncond.view(n_frames, h, num_selected_tokens, dim // h)
            v_uncond = v_uncond.view(n_frames, h, num_selected_tokens, dim // h)
            ...
            k_cond = k_cond.view(n_frames, h, num_selected_tokens, dim // h)
            v_cond = v_cond.view(n_frames, h, num_selected_tokens, dim // h)
            ...
            for frame in range(0, n_frames, b):
                ...
                out_cond = []
                for j in range(h):
                    sim_source_b = F.scaled_dot_product_attention(q_src[frame: frame + b, j],
                                                                  k_src[frame: frame + b, j],
                                                                  v_src[frame: frame + b, j],
                                                                  attn_mask=None,
                                                                  dropout_p=0.0)
                    sim_uncond_b = F.scaled_dot_product_attention(q_uncond[frame: frame + b, j],
                                                                  k_uncond[frame: frame + b, j],
                                                                  v_uncond[frame: frame + b, j],
                                                                  attn_mask=None,
                                                                  dropout_p=0.0)
                    sim_cond = F.scaled_dot_product_attention(q_cond[frame: frame + b, j],
                                                              k_cond[frame: frame + b, j],
                                                              v_cond[frame: frame + b, j],
                                                              attn_mask=None,
                                                              dropout_p=0.0)

                    out_source.append(sim_source_b)
                    out_uncond.append(sim_uncond_b)
                    out_cond.append(sim_cond)
...
def make_adaflow_attention_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    class AdaFlowBlock(block_class):
        ...
        def forward(
            ...
            if self.pivotal_pass:
                ...
                #     # print("Size: ", sequence_length ** 0.5)
                #     # print("idx1", idx1[0][-1] // (sequence_length ** 0.5), idx1[0][-1] % (sequence_length ** 0.5))
                #     # print("idx2", idx2[0][-1] // (sequence_length ** 0.5), idx2[0][-1] % (sequence_length ** 0.5))
                # else:
                ...
                for (h, w) in self.dift_size:
                    # print("h, w: ", h, w)
                    if sequence_length == h * w:
                        size = f'{h}_{w}'
                        break
                # print("Size: ", size)
                if len(batch_idxs) == 2:
                    idx1.append(self.matching[f'{size}_{batch_idxs[0]}'][
                                    int(self.pivotal_idx[batch_idxs[0]] % n_frames)])  # n_frames * seq_len
                    idx2.append(
                        self.matching[f'{size}_{batch_idxs[1]}'][int(self.pivotal_idx[batch_idxs[1]] % n_frames)])
            ...
            if not self.pivotal_pass:
                if len(batch_idxs) == 2:
                    ...
                    p1 = int(self.pivotal_idx[batch_idxs[0]])
                    p2 = int(self.pivotal_idx[batch_idxs[1]])
                    d1 = torch.abs(s - p1)
                    d2 = torch.abs(s - p2)
                    # weight
                    w1 = d2 / (d1 + d2)
...
def set_adaflow(
    ...
    """
    Sets the adaflow attention blocks in a model.
    """

    for _, module in model.named_modules():
        if isinstance_str(module, "BasicTransformerBlock"):
            make_adaflow_block_fn = make_adaflow_attention_block
            module.__class__ = make_adaflow_block_fn(module.__class__)
