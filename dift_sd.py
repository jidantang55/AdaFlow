...
class SDFeaturizer:
    def __init__(self, sd_id='stabilityai/stable-diffusion-2-1-base', null_prompt=''):
        unet = MyUNet2DConditionModel.from_pretrained(sd_id, subfolder="unet", revision="fp16", torch_dtype=torch.float16)
        onestep_pipe = OneStepSDPipeline.from_pretrained(sd_id, unet=unet, safety_checker=None, revision="fp16", torch_dtype=torch.float16)
    ...
    def forward(self,
        ...
        '''
        if type(up_ft_index) is not list:
        ...
        for i in up_ft_index:
            unet_ft[i] = unet_ft[i].mean(0, keepdim=True)  # 1,c,h,w
        return unet_ft
